-- Renumber every operator to F3X-NNNN by registration order.
-- KRISO (earliest created_at) becomes F3X-0001, the next one F3X-0002, etc.
-- All FK-bearing rows get their references rewritten in lock-step.

BEGIN;

-- 1. Build the old_id → new_id mapping ordered by registration time.
CREATE TEMP TABLE _op_id_map AS
SELECT id AS old_id,
       'F3X-' || LPAD((ROW_NUMBER() OVER (ORDER BY created_at ASC, id ASC))::text, 4, '0') AS new_id
FROM public.operators;

-- 2. Drop every FK that points at operators(id). Names follow Postgres
--    default <table>_<column>_fkey when the table was created with the
--    inline `references public.operators(id)` shorthand. IF EXISTS keeps
--    this idempotent.
...
-- 3. Rewrite every reference to use new IDs.
...
-- These tables exist from later migrations; guard against missing columns.
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='entry_read_log') THEN
    EXECUTE 'UPDATE public.entry_read_log erl SET operator_id = m.new_id FROM _op_id_map m WHERE erl.operator_id = m.old_id';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='admin_audit_log' AND column_name='actor_id') THEN
    EXECUTE 'UPDATE public.admin_audit_log al SET actor_id = m.new_id FROM _op_id_map m WHERE al.actor_id = m.old_id';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='admin_audit_log' AND column_name='target_id') THEN
    EXECUTE 'UPDATE public.admin_audit_log al SET target_id = m.new_id FROM _op_id_map m WHERE al.target_id = m.old_id';
  END IF;
END $$;

-- 4. Finally renumber operators themselves.
UPDATE public.operators o SET id = m.new_id FROM _op_id_map m WHERE o.id = m.old_id;

-- 5. Recreate the FKs with their original names + behavior.
ALTER TABLE public.entries
  ADD CONSTRAINT entries_operator_id_fkey
...
ALTER TABLE public.profile_signals
  ADD CONSTRAINT profile_signals_author_id_fkey
  FOREIGN KEY (author_id) REFERENCES public.operators(id);

ALTER TABLE public.profile_signals
  ADD CONSTRAINT profile_signals_target_id_fkey
  FOREIGN KEY (target_id) REFERENCES public.operators(id) ON DELETE CASCADE;

ALTER TABLE public.entry_reactions
  ADD CONSTRAINT entry_reactions_operator_id_fkey
  FOREIGN KEY (operator_id) REFERENCES public.operators(id) ON DELETE CASCADE;

ALTER TABLE public.signal_reactions
  ADD CONSTRAINT signal_reactions_operator_id_fkey
  FOREIGN KEY (operator_id) REFERENCES public.operators(id) ON DELETE CASCADE;

ALTER TABLE public.profile_signal_reactions
  ADD CONSTRAINT profile_signal_reactions_operator_id_fkey
  FOREIGN KEY (operator_id) REFERENCES public.operators(id) ON DELETE CASCADE;

ALTER TABLE public.friendships
  ADD CONSTRAINT friendships_requester_id_fkey
  FOREIGN KEY (requester_id) REFERENCES public.operators(id) ON DELETE CASCADE;

ALTER TABLE public.friendships
  ADD CONSTRAINT friendships_addressee_id_fkey
  FOREIGN KEY (addressee_id) REFERENCES public.operators(id) ON DELETE CASCADE;

ALTER TABLE public.messages
  ADD CONSTRAINT messages_sender_id_fkey
  FOREIGN KEY (sender_id) REFERENCES public.operators(id) ON DELETE CASCADE;

ALTER TABLE public.messages
  ADD CONSTRAINT messages_receiver_id_fkey
  FOREIGN KEY (receiver_id) REFERENCES public.operators(id) ON DELETE CASCADE;

DROP TABLE _op_id_map;

-- 6. Sequence + RPC for issuing the next sequential operator ID.
CREATE SEQUENCE IF NOT EXISTS public.operator_seq;

-- Sync sequence to current MAX so the next register() call gets MAX+1.
SELECT setval(
  'public.operator_seq',
  GREATEST(
    1,
    COALESCE(
      (SELECT MAX((regexp_match(id, '^F3X-(\d+)$'))[1]::int) FROM public.operators WHERE id ~ '^F3X-\d+$'),
      0
    )
  )
);
...
SECURITY DEFINER
AS $$
  SELECT 'F3X-' || LPAD(NEXTVAL('public.operator_seq')::text, 4, '0')
$$;

-- Lock the function so only the service role can call it
